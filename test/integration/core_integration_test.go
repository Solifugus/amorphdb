...
func testProtocolOperations(t *testing.T, client *CoreTestClient) {
	...
	err := client.Write([]string{"world", "agent", "1000", "protocol", "test", "value"}, textValue, 1000)
	if err != nil {
		t.Fatalf("Failed to write via protocol: %v", err)
	}

	// Read it back via protocol
	readValue, err := client.Read([]string{"world", "agent", "1000", "protocol", "test", "value"})
	if err != nil {
...
func testDataPersistenceAcrossRestart(t *testing.T, dataDir string) {
	...
	err := client1.Write([]string{"world", "agent", "1000", "persistent", "data"}, persistentValue, 1000)
	if err != nil {
	...
	readValue, err := client2.Read([]string{"world", "agent", "1000", "persistent", "data"})
	if err != nil {
...
func testReadWriteProtocol(t *testing.T, client *CoreTestClient) {
	...
	err := client.Write([]string{"world", "agent", "1000", "proto", "text"}, textVal, 1000)
	if err != nil {
		t.Fatalf("Protocol write failed: %v", err)
	}

	readVal, err := client.Read([]string{"world", "agent", "1000", "proto", "text"})
	if err != nil {
	...
	err = client.Write([]string{"world", "agent", "1000", "proto", "number"}, numVal, 1000)
	if err != nil {
		t.Fatalf("Protocol number write failed: %v", err)
	}

	readNumVal, err := client.Read([]string{"world", "agent", "1000", "proto", "number"})
	if err != nil {
