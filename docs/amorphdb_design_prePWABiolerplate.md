...
1. A user clicks a button in the PWA → client sends an MCP message
2. The Go layer writes the action to the client's session space in the
   data tree (e.g., `my.app.client.{session}.actions.submit = true`)
3. A watcher on that path fires and executes business logic
4. The watcher writes results to the data tree
5. The Go layer detects the change and pushes it to the client via SSE

The MBL developer writes watchers on application data, not on HTTP requests:

```
# React to a user action — this is a data change, not an HTTP request
watch my.app.handle_submit(my.app.client.current.actions.submit):
    if my.app.client.current.actions.submit = true:
        my.app.orders..append(my.app.client.current.forms.order_data)
        my.app.client.current.actions.submit = (quietly) false
```

This approach means:
- The MBL developer never thinks about HTTP, request objects, or response codes
- Real-time updates flow automatically — any data change pushes to subscribed clients
- Multiple users see each other's changes immediately
- The Go layer handles all transport concerns (SSE connections, MCP parsing, session management)
...
provides six primitives — transport (SSE + HTTP POST), message router, local
data cache, data-driven renderer, server-side watcher bridge, and deep link
mapping. Everything else is application logic.

- Vanilla HTML/CSS/JavaScript (no framework dependencies)
- Data-driven communication — PWA clients interact through data changes, not HTTP requests
- MCP messages over SSE (server→client) and HTTP POST (client→server)
- Go layer translates between MCP and the application's data tree
- MBL developer writes watchers on application data, not on HTTP requests
- SPA routing with deep-linkable URLs
- Server-side watcher bridge as security boundary
